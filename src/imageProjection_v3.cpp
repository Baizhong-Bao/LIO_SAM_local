...
std::vector<cv::Scalar> color = {
	cv::Scalar(250, 250, 255), cv::Scalar(255, 255, 187), cv::Scalar(220, 220, 220), cv::Scalar(205, 205, 150),
	cv::Scalar(230, 245, 253), cv::Scalar(255, 245, 152), cv::Scalar(205, 235, 255), cv::Scalar(205, 197, 0),
	cv::Scalar(255, 222, 173), cv::Scalar(255, 255, 0), cv::Scalar(205, 250, 255), cv::Scalar(255, 255, 151),
	cv::Scalar(238, 245, 255), cv::Scalar(238, 238, 141), cv::Scalar(240, 255, 240), cv::Scalar(205, 205, 121),
	cv::Scalar(240, 255, 255), cv::Scalar(212, 255, 127), cv::Scalar(250, 230, 230), cv::Scalar(170, 205, 102),
	cv::Scalar(245, 240, 255), cv::Scalar(166, 139, 69), cv::Scalar(225, 228, 255), cv::Scalar(193, 255, 193),
	cv::Scalar(79, 79, 47), cv::Scalar(105, 139, 105), cv::Scalar(153, 136, 119), cv::Scalar(128, 205, 67),
	cv::Scalar(112, 25, 25), cv::Scalar(144, 238, 144), cv::Scalar(237, 149, 100), cv::Scalar(84, 139, 84),
	cv::Scalar(139, 61, 72), cv::Scalar(127, 255, 0), cv::Scalar(238, 104, 123), cv::Scalar(255, 105, 65),
	cv::Scalar(255, 144, 30), cv::Scalar(0, 139, 0), cv::Scalar(235, 206, 135), cv::Scalar(0, 238, 118),
	cv::Scalar(180, 130, 70), cv::Scalar(0, 139, 69), cv::Scalar(222, 196, 176), cv::Scalar(62, 255, 192),
	cv::Scalar(230, 244, 176), cv::Scalar(50, 205, 154), cv::Scalar(209, 206, 0), cv::Scalar(112, 255, 202),
	cv::Scalar(208, 224, 64), cv::Scalar(90, 205, 162), cv::Scalar(160, 158, 95), cv::Scalar(133, 230, 238),
	cv::Scalar(170, 232, 238), cv::Scalar(29, 2155, 205), cv::Scalar(130, 221, 238), cv::Scalar(8, 101, 139),
	cv::Scalar(143, 143, 188), cv::Scalar(155, 155, 205), cv::Scalar(25, 82, 160), cv::Scalar(99, 99, 238),
	cv::Scalar(63, 133, 205), cv::Scalar(85, 85, 205), cv::Scalar(179, 222, 245), cv::Scalar(66, 121, 238),
	cv::Scalar(80, 127, 255), cv::Scalar(79, 165, 255), cv::Scalar(128, 128, 240), cv::Scalar(43, 90, 139),
	cv::Scalar(211, 85, 186), cv::Scalar(98, 130, 238), cv::Scalar(240, 32, 160), cv::Scalar(144, 149, 238),
	cv::Scalar(216, 191, 216), cv::Scalar(66, 87, 139), cv::Scalar(240, 255, 255), cv::Scalar(197, 181, 255),
	cv::Scalar(123, 125, 139), cv::Scalar(98, 28, 139), cv::Scalar(255, 111, 131), cv::Scalar(201, 105, 205)
};
...
private:
    ...
    ros::Subscriber subRightImage;
    std::deque<sensor_msgs::Image> leftImageQue;
    std::deque<sensor_msgs::Image> rightImageQue;

    std::mutex leftImageLock;
    std::mutex rightImageLock;
...
public:
    ...
    deskewFlag(0)
    {
        ...
        subLeftImage  = nh.subscribe<sensor_msgs::Image>("kitti/camera_color_left/image_raw", 10, &ImageProjection::leftImageHandler, this, ros::TransportHints().tcpNoDelay());
        subRightImage = nh.subscribe<sensor_msgs::Image>("kitti/camera_color_right/image_raw", 10, &ImageProjection::rightImageHandler, this, ros::TransportHints().tcpNoDelay());
        subLaserCloud = nh.subscribe<sensor_msgs::PointCloud2>(pointCloudTopic, 5, &ImageProjection::cloudInfoImagehandler, this, ros::TransportHints().tcpNoDelay());
    ...
    void allocateMemory()
    {
        ...
        semantic.reset(new pcl::PointCloud<PointXYZIRT>());
    ...
    bool cachePointCloud(const sensor_msgs::PointCloud2ConstPtr& laserCloudMsg)
    {
        ...
        imgLeft = std::move(leftImageQue.front());
        imgRight = std::move(rightImageQue.front());
        leftImageQue.pop_front();
        rightImageQue.pop_front();
    ...
    void projectPointCloud()
    {
        ...
        pcl::PointIndices::Ptr semanticIndices(new pcl::PointIndices());
        ...
        for (int i = 0; i < cloudSize; i++)
        {
            ...
            if (cls < 11) {
                ...
                // if (abs(depthVal - point.x) < 1.5)
                semanticIndices->indices.push_back(i);
            }
        }
        extract.setInputCloud(laserCloudIn);
        
        extract.setIndices(semanticIndices);
        extract.setNegative(false);
        extract.filter(*semantic);
    ...
    void publishClouds()
    {
        ...
        cloudInfo.cloud_semantic = publishCloud(pubSemanticCloud, semantic, cloudHeader.stamp, lidarFrame);
        pubLaserCloudInfo.publish(cloudInfo);
    }

    void cloudInfoImagehandler(const sensor_msgs::PointCloud2ConstPtr& msgIn)
    {
        if (leftImageQue.empty() || rightImageQue.empty())
        {
            return;
        }
    ...
    void leftImageHandler(const sensor_msgs::ImageConstPtr& leftImage)
    {
    ...
    {
        // std::cout << "Debug: " << std::endl;
        std::lock_guard<std::mutex> lock4(rightImageLock);
        rightImageQue.push_back(*rightImage);
        if (rightCount % 10 == 0)
        std::cout << "Debug: right image count: " << ++rightCount << std::endl;
    }
    // LetterBox处理
